...
func GetItemFromCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// involves aggregation pipeline to get the sum of all the items in the user cart
		...
		for _, json := range listing {
			c.IndentedJSON(200, json["total"])       // total sum of all the products in cart
			c.IndentedJSON(200, filledcart.UserCart) // all the products in the cart
		}
		ctx.Done()
	}
}
